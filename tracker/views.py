...
def register_view(request):
    if request.method == "POST":
        ...
        user = User.objects.filter(username = username)
        if user.exists():
            messages.success(request,'Pick another username')
        ...
        user.save()
        messages.success(request,'Account created and please login')
...
def index(request):
    if request.method == "POST":
        ...
        if amount is None or description is None:
            messages.success(request, "Please enter a description and amount")
            return redirect('/')
